...
knitr::opts_chunk$set(echo = FALSE, fig.pos = 'h', message = FALSE, warning = FALSE, fig.height = 5)
```
...
```{r, fig.cap = "Serie de tiempo, cantidad de ecooperativas fundadas cada año"}
inno_arr <- datos %>%
...
inno_arr %>%
  ...
  summarise(n = n()) %>%
  ggplot() +
  geom_line(aes(x = a4, y = n, color = cp)) +
  labs(x = "Año", y = "Organizaciones fundadas", color = "Tipo de organización") +
  scale_color_brewer(palette = "Set1") 
  
```
Hay empresas con mas antigüedad que la cooperativa con más años desde su inicio de actividades. Hay una tendencia acelerada de crecimiento en los dos tipos de organizaciones luego del año 2000, la que tal vez se corresponda con crecimiento de la economía en dicho periodo (posterior a la crisis de principios de siglo).
...
inno_arr %>%
  ...
  filter(hubo != "No") %>%
  ggplot() +
  geom_line(aes(x = a4, y = prop, color = tipo)) +
  facet_wrap(vars(cp)) + 
...
Se puede ver que hay una correspondencia entre aquellos que se reportan muy impacientes o muy pacientes y quienes reportan que el ausentismo es un problema en su cooperativa. Claramente no podemos afirmar causalidad y además hay muchos factores que no estamos tomando en cuenta, pero nos parece interesante esta correspondencia.
...
```{r, fig.cap = "Gráfico de dispersión, total de ingresos según inversión"}
datos %>%
...
```
Para las cooperativas los datos resultan escasos para afirmar una relación entre las variables. En la Figura gráfico se puede ver las relación entre el total de inversión de las empresas y su correspondiente nivel de ingresos. No se puede ver una correlación lineal positiva , lo que se podría interpretar como que no es claro que una mayor inversión resulte en un mayor monto de ingresos.


# Descripción de la aplicación Shiny

En nuestra aplicación queremos expandir sobre la pregunta 6 de nuestro análisis exploratorio, haciendo posible elegir las variables de un mapa de calor donde se muestra el desempeño en un área de el funcionamiento de la actividad económica de la cooperativa según el de otra. Se muestra la proporción de cooperativas en cada uno de los cuadrantes que cruzan los niveles de desempeño (mucho peor, peor, igual, mejor, mucho mejor) en comparación a empresas de similar tamaño.
...
```{r, fig.cap = "Gráfico de barras, proporción de organizaciones que realizaron al menos una innovación en producto en los últimos 3 años según año de inicio de sus actividades"}
inno_arr %>% 
  ggplot() +
